...
{
    public class GetPurchaseOrdersListQueryHandler : IRequestHandler<GetPurchaseOrdersListQuery, List<PurchaseOrdersListVm>>
    {
        ...
        public async Task<List<PurchaseOrdersListVm>> Handle(GetPurchaseOrdersListQuery request, CancellationToken cancellationToken)
        {
            var allPurchaseOrders = (await _purchaseOrderRepository.ListAllAsync()).OrderBy(x => x.PlacedOn);
            return _mapper.Map<List<PurchaseOrdersListVm>>(allPurchaseOrders);
        }
    }
}
